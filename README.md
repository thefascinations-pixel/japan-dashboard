# Japan Life Dashboard

A luxury dark-themed expat dashboard for living in Tokyo. Built with Next.js 14, powered by Claude AI.

## Features

- **Live Tokyo Weather** - Current conditions + 4-day forecast via Open-Meteo (free, no API key needed)
- **Currency Converter** - Real-time JPY to USD, EUR, IDR, GBP, AUD, KRW
- **Hana AI Assistant** - Claude-powered Japan life guide (visas, transit, food, culture, language)
- **Tokyo Clock** - Live Tokyo time + date
- **Emergency Numbers** - Police, ambulance, English helplines
- **Public Holidays** - Upcoming Japanese holidays
- **Expat Links** - Curated links: immigration, transit, daily life, community

## Setup

### 1. Clone and install

```bash
git clone <your-repo>
cd japan-dashboard
npm install
```

### 2. Set environment variables

```bash
cp .env.example .env.local
# Edit .env.local and add your Anthropic API key
```

Get your Anthropic API key at: https://console.anthropic.com/

### 3. Run locally

```bash
npm run dev
# Open http://localhost:3000
```

## Deploy to Vercel

### Option A - Vercel CLI

```bash
npm i -g vercel
vercel
# Follow prompts, then add env var:
vercel env add ANTHROPIC_API_KEY
```

### Option B - Vercel Dashboard

1. Push to GitHub.
2. Go to [vercel.com](https://vercel.com) and import the project.
3. Add `ANTHROPIC_API_KEY` in Settings -> Environment Variables.
4. Deploy.

## Tech Stack

- **Framework**: Next.js 14 (App Router)
- **AI**: Anthropic Claude via `/api/chat`
- **Weather**: Open-Meteo API
- **Currency**: Exchange rate API via `/api/currency`
- **Fonts**: Shippori Mincho + DM Sans
- **Styling**: CSS Modules

## Environment Variables

| Variable | Required | Description |
|----------|----------|-------------|
| `ANTHROPIC_API_KEY` | Yes | Your Anthropic API key for Hana AI chat |

## Structure

```text
app/
  page.tsx
  layout.tsx
  globals.css
  api/
    chat/route.ts
    weather/route.ts
    currency/route.ts
components/
  WeatherWidget.tsx
  CurrencyWidget.tsx
  ChatWidget.tsx
  InfoWidgets.tsx
  QuickFacts.tsx
```
